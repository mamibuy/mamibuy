    ...
    <div>
      ...
      {detailModal && (() => {
        ...
        const updateItem = (i, field, val) => {
          ...
          setEf2("items", items);
          setEf2("taxAmount", totalTax);
          setEf2("amount", totalUntax);
        };
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #F0F0F0", gap:12 }}>
            ...
            {options ? (
              ...
              <input style={{ ...inp, padding:"4px 8px", fontSize:13, flex:1, textAlign:type==="number"?"right":"left" }}
                ...
                value={ef2[field]??""} onChange={e=>{
                  if(type==="number"){
                    const v=parseInt(e.target.value.replace(/\D/g,""),10);
                    setEf2(field, isNaN(v)?"":v);
                  } else { setEf2(field, e.target.value); }
                }}
              />
            )}
          </div>
        ...
        const saveModalEdit = async () => {
          ...
          const merged = { ...dm, ...ef2 };
          const clean = {
            id: merged.id, applicant: merged.applicant, company: merged.company,
            title: merged.title, clientTitle: merged.clientTitle||"",
            taxId: merged.taxId||"", orderNo: merged.orderNo||"",
            invoiceNo: merged.invoiceNo||"", invoiceDate: nullIfEmpty(merged.invoiceDate),
            expectedPayDate: nullIfEmpty(merged.expectedPayDate),
            status: merged.status,
            amount: numOrNull(merged.amount), taxAmount: numOrNull(merged.taxAmount),
            paidDate: nullIfEmpty(merged.paidDate),
            paidAmount: numOrNull(merged.paidAmount),
            bankFee: numOrNull(merged.bankFee),
            commission: numOrNull(merged.commission),
            commissionNo: merged.commissionNo||"",
            expenseNo: merged.expenseNo||"",
            voucherNo: merged.voucherNo||"",
            vendorId: merged.vendorId||null,
            paid: merged.paid||0,
            theme: merged.theme||"invoice",
            items: merged.items||[],
          };
          try {
          ...
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center", padding:0 }}
            ...
            <div style={{ background:"#fff", borderRadius:"20px 20px 0 0", padding:"0 0 32px", width:"100%", maxWidth:560, maxHeight:"92vh", overflowY:"auto", boxShadow:"0 -8px 40px rgba(0,0,0,0.18)" }}>
                    ...
                    {isEditingModal && (
                      <>
                        <button onClick={saveModalEdit}
                          style={{ border:"none", background:"#1a1a2e", color:"#fff", borderRadius:8, padding:"6px 14px", fontSize:13, cursor:"pointer", fontWeight:700 }}>
                          💾 儲存
                        </button>
                        <button onClick={()=>setDetailModal(prev=>({...prev,_editing:false,_editForm:{}}))}
                          style={{ border:"1px solid #ddd", background:"#fff", color:"#666", borderRadius:8, padding:"6px 12px", fontSize:13, cursor:"pointer" }}>
                          取消
                        </button>
                      </>
                    )}
                    <button onClick={()=>setDetailModal(null)}
                      style={{ border:"none", background:"#F5F5F5", borderRadius:"50%", width:36, height:36, cursor:"pointer", fontSize:18, color:"#666", display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding:"0 24px" }}>
                {/* 申請人資訊 */}
                <Section title="申請人資訊">
                  <Row label="申請人" value={dm.applicant}/>
                  <Row label="公司別" value={dm.company}/>
                </Section>

                {/* 款項資訊 */}
                <Section title="款項資訊" color="#5C85D6">
                  {isEditingModal ? <>
                    <ERow label="公司抬頭" field="clientTitle"/>
                    <ERow label="統一編號" field="taxId"/>
                    <ERow label="委刊單編號" field="orderNo"/>
                    <ERow label="未稅金額" field="amount" type="number"/>
                    <ERow label="含稅金額" field="taxAmount" type="number"/>
                  </> : <>
                    <Row label="公司抬頭" value={dm.clientTitle}/>
                    <Row label="統一編號" value={dm.taxId} mono/>
                    ...
                    {editItems.map((it,i)=>(
                      <div key={i} style={{ borderBottom:"1px solid #F0F0F0", padding:"10px 0" }}>
                        <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:6 }}>
                          <select style={{ ...inp, flex:2, padding:"5px 8px", fontSize:13 }}
                          ...
                          <button onClick={()=>{ const items=editItems.filter((_,j)=>j!==i); const totalTax=items.reduce((s,x)=>s+(x.taxAmt||0),0); const totalUntax=items.reduce((s,x)=>s+(x.untaxAmt||0),0); setEf2("items",items); setEf2("taxAmount",totalTax); setEf2("amount",totalUntax); }}
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6 }}>
                          <div>
                            <div style={{ fontSize:11, color:"#aaa", marginBottom:2 }}>數量</div>
                            <input style={{ ...inp, padding:"5px 8px", fontSize:13, textAlign:"right" }}
                              type="text" inputMode="numeric"
                              value={it.qty||""} onChange={e=>{ const v=parseInt(e.target.value.replace(/\D/g,""),10); updateItem(i,"qty",isNaN(v)?1:v); }}/>
                          </div>
                          <div>
                            <div style={{ fontSize:11, color:"#aaa", marginBottom:2 }}>單價（未稅）</div>
                            <input style={{ ...inp, padding:"5px 8px", fontSize:13, textAlign:"right" }}
                              type="text" inputMode="numeric"
                              value={it.unitPrice||""} onChange={e=>{ const v=parseInt(e.target.value.replace(/\D/g,""),10); updateItem(i,"unitPrice",isNaN(v)?0:v); }}/>
                          </div>
                          <div>
                            <div style={{ fontSize:11, color:"#aaa", marginBottom:2 }}>含稅小計</div>
                            <div style={{ ...inp, padding:"5px 8px", fontSize:13, textAlign:"right", background:"#F0FFF4", color:"#2E7D32", fontWeight:700 }}>{it.taxAmt?it.taxAmt.toLocaleString():"-"}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                    <button onClick={()=>setEf2("items",[...editItems,{label:"",qty:1,unitPrice:0,untaxAmt:0,taxAmt:0}])}
                      style={{ width:"100%", marginTop:10, padding:"8px", border:"2px dashed #00897B", background:"#F0FDF4", color:"#00897B", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:600 }}>
                      ＋ 新增品項
                    </button>
                    <div style={{ marginTop:8, textAlign:"right", fontSize:13, color:"#2E7D32", fontWeight:700 }}>
                      合計（含稅）：NT$ {(ef2.taxAmount||0).toLocaleString()}
                    </div>
                  </Section>
                )}
                {/* 品項明細 - 檢視模式 */}
                {!isEditingModal && (() => {
                  ...
                  const fallbackItems = !hasItems && dm.title
                    ? dm.title.split("、").map(label => ({ label: label.trim() }))
                    : [];
                  ...
                  return (
                    <Section title="品項明細" color="#00897B">
                      <div style={{ overflowX:"auto" }}>
                        ...
                        {!hasItems && (
                          <div style={{ fontSize:11, color:"#bbb", padding:"6px 8px" }}>此筆為舊資料，僅顯示品項名稱，無數量與金額明細</div>
                        )}
                      </div>
                    </Section>
                  );
                ...
                <Section title="發票資訊" color="#43A047">
                  {isEditingModal ? <>
                    <ERow label="發票號碼" field="invoiceNo" mono/>
                    <ERow label="傳票號碼" field="voucherNo" mono/>
                    <ERow label="發票日期" field="invoiceDate" type="date"/>
                    <ERow label="預計收款日" field="expectedPayDate" type="date"/>
                    <ERow label="付款狀態" field="status" options={["申請中","已開立","到期未付款","已付款"]}/>
                  </> : <>
                    <Row label="發票號碼" value={dm.invoiceNo} mono/>
                    <Row label="傳票號碼" value={dm.voucherNo} mono/>
                    <Row label="發票日期" value={dm.invoiceDate}/>
                    <Row label="預計收款日" value={dm.expectedPayDate}/>
                  </>}
                </Section>
                ...
                <Section title="付款資訊" color="#F57C00">
                  {isEditingModal ? <>
                    <ERow label="已收款金額" field="paidAmount" type="number"/>
                    <ERow label="實際收款日" field="paidDate" type="date"/>
                    <ERow label="手續費" field="bankFee" type="number"/>
                    <ERow label="佣金" field="commission" type="number"/>
                    <ERow label="說明" field="commissionNo"/>
                    <ERow label="支出申請單" field="expenseNo"/>
                  </> : <>
                    <Row label="已收款金額" value={dm.paidAmount?`NT$ ${dm.paidAmount.toLocaleString()}`:null}/>
                    <Row label="實際收款日" value={dm.paidDate}/>
                    <Row label="手續費" value={dm.bankFee?`NT$ ${dm.bankFee.toLocaleString()}`:null}/>
                    <Row label="佣金" value={dm.commission?`NT$ ${dm.commission.toLocaleString()}`:null}/>
                    <Row label="說明" value={dm.commissionNo}/>
                    <Row label="支出申請單" value={dm.expenseNo} mono/>
                  </>}
                </Section>
                ...
                {dv && (
                  <Section title="廠商付款資訊" color="#7B1FA2">
                    ...
                    <Row label="戶名" value={dv.accountName}/>
                    <Row label="帳號" value={dv.account} mono/>
                  </Section>
                )}
              </div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12, overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 4 }}>
        <span style={{ fontSize: 12, color: "#888", alignSelf: "center", marginRight: 4 }}>發票開立月份：</span>
        {months.map(m => (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, flexWrap: "wrap", rowGap: 8 }}>
        ...
        <input type="date" style={{ ...inp, width: "auto", fontSize: 13, padding: "5px 10px" }}
        ...
        {(dateFrom || dateTo) && (
          <span style={{ fontSize: 12, color: "#F2A0A0", fontWeight: 600 }}>
            共 {filtered.length} 筆
          </span>
        )}
      </div>
      ...
      {isMobile ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: "center", color: "#aaa", padding: 32 }}>尚無符合條件的紀錄</div>
          )}
          {filtered.map(p => {
            ...
            return (
              <div key={p.id}
                ...
                style={{ background: "#fff", borderRadius: 14, padding: "14px 16px", boxShadow: "0 2px 10px rgba(0,0,0,0.06)", border: "1px solid #F0F0F0", cursor: "pointer" }}>
                {/* 頂部：賣方 + 狀態 */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ background: "#E8EAF6", color: "#3949AB", padding: "2px 8px", borderRadius: 6, fontSize: 12, fontWeight: 600 }}>{p.company}</span>
                    <span style={{ fontSize: 12, color: "#888" }}>{p.applicant}</span>
                  </div>
                  <Badge status={p._effectiveStatus} />
                </div>
                {/* 買方 */}
                <div style={{ fontSize: 14, fontWeight: 600, color: "#1a1a2e", marginBottom: 4, lineHeight: 1.4 }}>{p.clientTitle || p.title}</div>
                {/* 金額 */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 13, color: "#888" }}>含稅金額</span>
                  <span style={{ fontSize: 18, fontWeight: 700, color: "#2E7D32" }}>{fmt(p.taxAmount)}</span>
                </div>
                {/* 統編 */}
                {p.taxId && <div style={{ fontSize: 12, color: "#888", marginBottom: 4 }}>統編：{p.taxId}</div>}
                {/* 日期資訊 */}
                <div style={{ display: "flex", gap: 12, fontSize: 12, color: "#888", marginBottom: 6, flexWrap: "wrap" }}>
                  {p.invoiceDate && <span>發票日期 {p.invoiceDate}</span>}
                  {p.expectedPayDate && <span>預計收款 {p.expectedPayDate}</span>}
                </div>
                {/* 發票號碼 + 已收款 */}
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 10, flexWrap: "wrap", gap: 6 }}>
                  {p.invoiceNo && <span style={{ fontFamily: "monospace", color: "#555" }}>{p.invoiceNo}</span>}
                  {p.paidAmount && <span style={{ color: "#2E7D32", fontWeight: 700 }}>已收 {fmt(p.paidAmount)}</span>}
                </div>
                {/* 操作按鈕列 */}
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: "1px solid #F5F5F5", paddingTop: 10 }}
                  onClick={e => e.stopPropagation()}>
                  {p._effectiveStatus === "申請中" && (
                    <button onClick={() => { setIssueModal({ id: p.id }); setInvoiceNo(""); }}
                      style={{ border: "1px solid #007BFF", background: "#EBF5FF", color: "#0056b3", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                      ✓ 標記已開立
                    </button>
                  )}
                  {(p._effectiveStatus === "已開立" || p._effectiveStatus === "到期未付款") && (
                    <button onClick={() => setProjects(projects.map(x => x.id === p.id ? { ...x, status: "已付款" } : x))}
                      style={{ border: "1px solid #28A745", background: "#EAFBF0", color: "#155724", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                      💰 已付款
                    </button>
                  )}
                  <button onClick={() => onCopyProject(p)}
                    style={{ border: "1px solid #F2A0A0", background: "#FFF5F5", color: "#c0566a", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                    📋 複製
                  </button>
                  {canEdit && (
                    <button onClick={() => setDetailModal({
                      ...
                      _editing: true,
                      _editForm: {
                        clientTitle: p.clientTitle, taxId: p.taxId, orderNo: p.orderNo,
                        invoiceNo: p.invoiceNo, invoiceDate: p.invoiceDate,
                        expectedPayDate: p.expectedPayDate, status: p.status,
                        paidDate: p.paidDate||"", paidAmount: p.paidAmount||"",
                        bankFee: p.bankFee||"", commission: p.commission||"",
                        commissionNo: p.commissionNo||"", expenseNo: p.expenseNo||"",
                        voucherNo: p.voucherNo||"", amount: p.amount, taxAmount: p.taxAmount,
                        items: p.items && p.items.length>0 ? p.items.map(i=>({...i})) : [{label:"",qty:1,unitPrice:0,untaxAmt:0,taxAmt:0}],
                      }
                    })}
                      style={{ border: "1px solid #ddd", background: "#F8F9FA", color: "#444", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>
                      ✏️ 編輯
                    </button>
                  )}
                  {canEdit && (
                    <button onClick={() => setDeleteConfirm(p.id)}
                      style={{ border: "1px solid #fcc", background: "#fff5f5", color: "#c33", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>
                      🗑 刪除
                    </button>
                  )}
                </div>
      ...
      <Card style={{ padding: 0, overflow: "visible" }}>
        <div className="rwd-scroll-container" style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 280px)", WebkitOverflowScrolling: "touch", borderRadius: 16 }}>
          <table style={{ borderCollapse: "collapse", fontSize: 13, tableLayout: isMobile ? "auto" : "fixed", width: isMobile ? "auto" : "100%", minWidth: isMobile ? 700 : 1710 }}>
            <thead>
              <tr style={{ background: "#F8F9FA" }}>
                    ...
                    {isEditing ? (
                      ...
                      <>
                        ...
                        <td style={{ ...sTd, left: 260, width: 100, minWidth: 100, background: rowBg, borderRight: "2px solid #D0D0D0" }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: 80 }} type="number"
                            value={ef.taxAmount} onChange={e => setEf("taxAmount", +e.target.value)} />
                        </td>
                        {/* 統編/身分證 */}
                        ...
                        <td style={{ ...td, width: 100, minWidth: 100 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }} type="date"
                            value={ef.invoiceDate} onChange={e => setEf("invoiceDate", e.target.value)} />
                        </td>
                        {/* 發票號碼 */}
                        ...
                        <td style={{ ...td, width: 100, minWidth: 100 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }} type="date"
                            value={ef.paidDate || ""} onChange={e => setEf("paidDate", e.target.value)} />
                        </td>
                        {/* 預計收款日 */}
                        <td style={{ ...td, width: 100, minWidth: 100 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }} type="date"
                            value={ef.expectedPayDate || ""} onChange={e => setEf("expectedPayDate", e.target.value)} />
                        </td>
                        {/* 付款狀態 */}
                        <td style={{ ...td, width: 120, minWidth: 120 }}>
                        ...
                        {/* 說明 */}
                        <td style={{ ...td, width: 110, minWidth: 110 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }}
                            value={ef.commissionNo || ""} onChange={e => setEf("commissionNo", e.target.value)} placeholder="說明" />
                        </td>
                        {/* 支出申請單編號 */}
                        <td style={{ ...td, width: 130, minWidth: 130 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }}
                            value={ef.expenseNo || ""} onChange={e => setEf("expenseNo", e.target.value)} placeholder="支出申請單編號" />
                        </td>
                        {/* 傳票號碼 */}
                        <td style={{ ...td, width: 110, minWidth: 110 }}>
                          <input style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }}
                            value={ef.voucherNo || ""} onChange={e => setEf("voucherNo", e.target.value)} placeholder="傳票號碼" />
                        </td>
                        {/* 申請人 */}
                        <td style={{ ...td, width: 90, minWidth: 90 }}>
                          <select style={{ ...inp, padding: "4px 6px", fontSize: 12, width: "100%" }} value={ef.applicant} onChange={e => setEf("applicant", e.target.value)}>
                            {!users.some(u => u.name === ef.applicant) && (
                              <option value={ef.applicant}>{ef.applicant}</option>
                            )}
                            {users.map(u => <option key={u.id} value={u.name}>{u.name}</option>)}
                          </select>
                        </td>
                        {/* 操作 */}
                        <td style={{ ...td, width: 110, minWidth: 110, whiteSpace: "nowrap" }}>
                      ...
                      <>
                        ...
                        <td style={{ ...td, whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            ...
                            {/* 複製申請：所有人都可以用 */}
                            <button
                              onClick={e => { e.stopPropagation(); onCopyProject(p); }}
...
export default function App() {
  ...
  const [prefillData, setPrefillData] = useState(null); // 複製申請帶入
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      const vendorUpdated = formData.vSelected && formData.vFields?.name;
      showToast(vendorUpdated ? "✅ 發票申請已提交，廠商資料已同步更新！" : "✅ 發票申請已提交！");
      setPage("projects");
    } catch(err) {
      ...
      <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#FDE8E8 0%,#FFF5F5 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px",fontFamily:"'Noto Sans TC','Microsoft JhengHei',sans-serif"}}>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:"36px 28px",width:"100%",maxWidth:400,boxShadow:"0 16px 60px rgba(242,160,160,0.25)"}}>
          ...
          <div style={{marginBottom:24}}>
            ...
            <div style={{position:"relative"}}>
              <input style={{...inp,paddingRight:44,fontSize:14}}
                type={loginPwShow?"text":"password"} value={loginPw}
              ...
              />
              <span onClick={()=>setLoginPwShow(v=>!v)}
                style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",cursor:"pointer",fontSize:16,color:"#aaa",userSelect:"none"}}>
                {loginPwShow?"🙈":"👁"}
              </span>
            </div>
          ...
          {loginError&&(
            <div style={{background:"#FFF0F0",border:"1px solid #FCC",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#c33",marginBottom:16}}>
              ⚠ {loginError}
            </div>
          )}
  ...
  const canAccess = (p) => {
    const perms = {
      dashboard:true,"invoice-form":true,projects:true,vendors:true,
  ...
  const navItems = [
    ...
    {id:"invoice-form",label:"申請發票",icon:"📄"},
    {id:"projects",label:"進度",icon:"🧾"},
    {id:"vendors",label:"廠商管理",icon:"🏢"},
    ...
    <div style={{minHeight:"100vh",background:"#F5F6FA",fontFamily:"'Noto Sans TC','Microsoft JhengHei',sans-serif"}}>
          ...
          <div style={{background:"#fff",borderRadius:16,padding:24,width:380,boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button disabled={!myPw.trim()}
                onClick={async()=>{
                  try {
                    await sb.from("users").eq("id",currentUser.id).update({password:myPw.trim()});
                    setUsers(prev=>prev.map(u=>u.id===currentUser.id?{...u,password:myPw.trim()}:u));
                    setCurrentUser(u=>({...u,password:myPw.trim()}));
                    setMyPwModal(false); setMyPw(""); setMyPwShow(false);
                    showToast("✅ 密碼已更新！");
                  } catch(err) { showToast("❌ 更新失敗："+err.message,"error"); }
                }}
                style={{flex:1,border:"none",background:myPw.trim()?"#F2A0A0":"#ccc",color:"#fff",padding:"11px",borderRadius:8,cursor:myPw.trim()?"pointer":"not-allowed",fontWeight:700}}>
                確認更新
              </button>
              <button onClick={()=>{setMyPwModal(false);setMyPw("");setMyPwShow(false);}}
                style={{flex:1,border:"1px solid #ddd",background:"#fff",padding:"11px",borderRadius:8,cursor:"pointer"}}>取消</button>
            </div>
          </div>
        </div>
      ...
      <div style={{background:THEMES[theme].color,color:"#fff",padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56,position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button className="navbar-pw-btn" onClick={()=>{setMyPwModal(true);setMyPw("");setMyPwShow(false);}}
            style={{background:"#ffffff33",color:"#fff",border:"none",borderRadius:8,padding:"4px 10px",fontSize:12,cursor:"pointer"}}>🔑 密碼</button>
      ...
      {toast&&(
        <div style={{position:"fixed",bottom:24,right:24,background:toast.type==="success"?"#1a1a2e":"#FF9800",color:"#fff",padding:"14px 20px",borderRadius:12,boxShadow:"0 4px 20px rgba(0,0,0,0.2)",fontSize:14,fontWeight:500,zIndex:999}}>
          {toast.msg}
        </div>
      )}
    </div>
