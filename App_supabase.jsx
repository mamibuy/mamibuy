...
function ProjectList({ projects, setProjects, vendors, currentUser, users, onExport, onCopyProject }) {
  ...
  const confirmMarkIssued = async () => {
    if (!invoiceNo.trim()) return;
    const targetId = issueModal.id;
    const updated = projects.map(p => p.id === targetId ? { ...p, status: "已開立", invoiceNo: invoiceNo.trim() } : p);
    setProjects(updated);
    setIssueModal(null);
    setInvoiceNo("");
    try {
  ...
  const deleteProject = async (id) => {
    ...
    try { await sb.from("projects").eq("id", id).delete(); }
    catch(err) { console.warn("DB sync failed (delete):", err.message); }
  };
                    ...
                    {isEditing ? (
                      <>
                        ...
                        <td style={{ ...td, width: 90, minWidth: 90 }}>
                          <select style={{ ...inp, padding: "5px 8px", fontSize: 12, width: "100%" }} value={ef.payMethod || ""} onChange={e => setEf("payMethod", e.target.value)}>
                            <option value="">-</option>
                            {["匯款","支票","關係人沖帳"].map(m => <option key={m}>{m}</option>)}
                          </select>
                        </td>
